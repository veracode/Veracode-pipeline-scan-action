...
export async function checkParameters(parameters: any): Promise<string> {
  ...
  let scanCommand: string =
    "java -jar pipeline-scan.jar -vid " +
    parameters.vid +
    " -vkey " +
    parameters.vkey;
  ...
  if (parameters.veracode_policy_name != "") {
    ...
    const queryparams =
      "?name=" + encodeURIComponent(parameters.veracode_policy_name);
    ...
    core.debug(
      "---- check-parameters.ts / checkParameters() - if veracode_policy_name is set - show parameters ----"
    );
    ...
    const response = await axios.request({
      method: "GET",
      headers: {
        Authorization: auth.generateHeader(
          path,
          "GET",
          apiUrl,
          cleanedID,
    ...
    core.debug("---- DEBUG OUTPUT START ----");
    core.debug(
      "---- check-parameters.ts / checkParameters() - find the policy via API----"
    );
    core.debug("---- Response Data ----");
    ...
    if (response.data.page.total_elements != "0") {
      if (response.data._embedded.policy_versions[0].type == "BUILTIN") {
        ...
        scanCommand +=
          ' --policy_name "' + parameters.veracode_policy_name + '"';
      } else if (
        response.data._embedded.policy_versions[0].type == "CUSTOMER"
      ) {
        core.debug("Custom Policy is required");
        core.debug(
          "Downloading custom policy file and setting policy to " +
            parameters.veracode_policy_name
        );
        ...
        policyCommand =
          ...
          parameters.veracode_policy_name +
          '"';
        const policyDownloadOutput = await getPolicyFile(
          policyCommand,
          parameters
        );
        ...
        core.debug("---- DEBUG OUTPUT START ----");
        core.debug(
          "---- check-parameters.ts / checkParameters() - if veracode_policy_name is set and custom policy is required ----"
        );
        core.debug("---- Policy Download command: " + policyCommand);
        core.debug("---- Policy Downlaod Output: " + policyDownloadOutput);
        core.debug("---- DEBUG OUTPUT END ----");

        var policyFileName = parameters.veracode_policy_name.replace(
          / /gi,
          "_"
        );
...
*/
  ...
  if (parameters.request_policy != "") {
    ...
    policyCommand =
      ...
      parameters.vkey +
      ' --request_policy "' +
      parameters.request_policy +
      '"';
    const policyDownloadOutput = await getPolicyFile(policyCommand, parameters);

    core.debug("---- DEBUG OUTPUT START ----");
    core.debug(
      "---- check-parameters.ts / checkParameters() - if request policy == true ----"
    );
    core.debug("---- Policy Download command: " + policyCommand);
    core.debug("---- Policy Downlaod Output: " + policyDownloadOutput);
    core.debug("---- DEBUG OUTPUT END ----");
  ...
  Object.entries(parameters).forEach(([key, value], index) => {
    if (
      ...
      key != "run_method" &&
      key != "request_policy" &&
      key != "veracode_policy_name" &&
      key != "artifact_name" &&
      value != ""
    ) {
      core.debug("---- DEBUG OUTPUT START ----");
      core.debug(
        "---- check-parameters.ts / checkParameters() - run full scan----"
      );
      ...
      if (
        key != "debug" &&
        key != "store_baseline_file" &&
        key != "store_baseline_file_branch" &&
        key != "create_baseline_from" &&
        key != "fail_build"
      ) {
        if (key == "include") {
  ...
  core.debug("---- DEBUG OUTPUT START ----");
  core.debug(
    "---- check-parameters.ts / checkParameters() - return value ----"
  );
  core.debug("---- Pipeline Scan Command: " + scanCommand);
  core.debug("---- DEBUG OUTPUT END ----");
